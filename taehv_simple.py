...
Based on the official implementation from https://github.com/madebyollin/taehv
Simplified for ComfyUI integration.
"""
import torch
...
class TAEHV(nn.Module):
    ...
    def __init__(self, checkpoint_path=None, state_dict=None, decoder_time_upscale=(True, True), decoder_space_upscale=(True, True, True)):
        """Initialize pretrained TAEHV from the given checkpoint or state dict.

        Args:
            checkpoint_path: path to weight file to load. taehv.pth for Hunyuan, taew2_1.pth for Wan 2.1.
            state_dict: alternatively, provide state dict directly (for ComfyUI integration)
        ...
        # Load weights from either checkpoint_path or state_dict
        if state_dict is not None:
            self.load_state_dict(self.patch_tgrow_layers(state_dict))
        elif checkpoint_path is not None:
            weights = torch.load(checkpoint_path, map_location="cpu", weights_only=True)
            self.load_state_dict(self.patch_tgrow_layers(weights))
