...
from .core import output_to_terminal_successful, output_to_terminal_error
...
class TAESD(nn.Module):
    ...
    def __init__(self, latent_channels=4, decoder_path=None):
        ...
        if decoder_path and os.path.exists(decoder_path):
            try:
                ...
                output_to_terminal_successful(f"TAESD | Loaded TAESD weights from {decoder_path}")
            except Exception as e:
                output_to_terminal_error(f"TAESD | Failed to load TAESD weights: {e}")
    ...
    def decode_latent_to_preview_image(self, preview_format, x0):
        ...
        try:
            ...
            output_to_terminal_error(f"TAESD | TAESD preview failed: {e}")
            # Return fallback
    ...
    def decode_latent_to_preview(self, x0):
        ...
        try:
            ...
            output_to_terminal_error(f"TAESD | TAESD decode failed: {e}")
            import traceback
            output_to_terminal_error(f"TAESD | Traceback: {traceback.format_exc()}")
...
def download_taesd_models():
    ...
    if not (REQUESTS_AVAILABLE or URLLIB_AVAILABLE):
        output_to_terminal_error("TAESD | No download libraries available")
        return False
    
    # Get vae_approx directory
    vae_approx_paths = folder_paths.get_folder_paths("vae_approx")
    if not vae_approx_paths:
        output_to_terminal_error("TAESD | vae_approx folder not found")
        return False
    ...
    output_to_terminal_successful("TAESD | Downloading TAESD models...")
    success_count = 0
    ...
    for model in models:
        ...
        if os.path.exists(filepath):
            output_to_terminal_successful(f"TAESD | {model['name']} already exists")
            success_count += 1
            continue
        
        try:
            output_to_terminal_successful(f"TAESD | Downloading {model['name']}...")
            ...
            output_to_terminal_successful(f"TAESD | Downloaded {model['name']}")
            success_count += 1
            
        except Exception as e:
            output_to_terminal_error(f"TAESD | Failed to download {model['name']}: {e}")
    
    output_to_terminal_successful(f"TAESD | Downloaded {success_count}/{len(models)} models")
...
def get_taesd_decoder_path(latent_channels):
    ...
    if not model_name:
        output_to_terminal_error(f"TAESD | No TAESD model available for {latent_channels} channels")
        return None
    ...
    if not os.path.exists(model_path):
        output_to_terminal_successful(f"TAESD | TAESD model {model_name} not found, downloading...")
...
def create_taesd_decoder(latent_channels):
    ...
    try:
        ...
        output_to_terminal_successful(f"TAESD | Created TAESD decoder for {latent_channels} channels on {device}")
        return taesd
        
    except Exception as e:
        output_to_terminal_error(f"TAESD | Failed to create TAESD decoder: {e}")
        return None
...
def override_comfyui_taesd():
    ...
    try:
        ...
        def custom_get_previewer(device, latent_format):
            ...
            try:
                output_to_terminal_successful(f"TAESD | Custom previewer called with device: {device}, latent_format: {type(latent_format)}")
                ...
                if hasattr(latent_format, 'latent_channels'):
                    ...
                    output_to_terminal_successful(f"TAESD | Found latent_channels: {channels}")
                    ...
                    if hasattr(factors, '__len__'):
                        ...
                        output_to_terminal_successful(f"TAESD | Inferred channels from RGB factors: {channels}")
                elif hasattr(latent_format, 'scale_factor'):
                    # Default assumption based on common formats
                    output_to_terminal_successful("TAESD | Using default 4 channels for unknown format")
                    channels = 4
                
                if channels and channels in [4, 8, 16, 64]:
                    output_to_terminal_successful(f"TAESD | Attempting to create TAESD decoder for {channels} channels")
                    
                    # Try to get our TAESD decoder
                    taesd_decoder = get_or_create_taesd_decoder(channels)
                    
                    if taesd_decoder is not None:
                        output_to_terminal_successful(f"TAESD | Successfully created custom TAESD for {channels} channels")
                        return taesd_decoder
                    else:
                        output_to_terminal_error(f"TAESD | Failed to create TAESD decoder for {channels} channels")
                else:
                    output_to_terminal_error(f"TAESD | Unsupported channel count: {channels}. Supported: 4, 8, 16, 64")
                    if hasattr(latent_format, '__dict__'):
                        output_to_terminal_error(f"TAESD | Latent format attributes: {latent_format.__dict__}")
                
                # Fallback to original previewer
                output_to_terminal_successful("TAESD | Falling back to original previewer")
                return latent_preview._original_get_previewer(device, latent_format)
                
            except Exception as e:
                output_to_terminal_error(f"TAESD | Custom previewer failed: {e}")
                import traceback
                output_to_terminal_error(f"TAESD | Traceback: {traceback.format_exc()}")
                return latent_preview._original_get_previewer(device, latent_format)
        ...
        output_to_terminal_successful("TAESD | ComfyUI TAESD system overridden successfully")
        return True
        
    except Exception as e:
        output_to_terminal_error(f"TAESD | Failed to override ComfyUI TAESD: {e}")
        return False
...
def initialize_taesd_override():
    ...
    try:
        ...
        output_to_terminal_successful("TAESD | TAESD override system initialized")
        return True
        
    except Exception as e:
        output_to_terminal_error(f"TAESD | Failed to initialize TAESD override: {e}")
        return False
